...
const App: React.FC = () => {
  ...
  const handleSaveApiKey = async (apiKey: string) => {
    ...
    try {
        ...
        // A simple check to see if the key is valid by making a dummy request
  ...
  const handleResetApiKey = () => {
    if (window.confirm("Are you sure you want to reset your API key? This will clear the saved key and reload the application.")) {
        localStorage.removeItem('geminiApiKey');
  ...
  const handleSendMessage = async (text: string, isSystemTrigger = false) => {
    ...
    try {
      ...
      let errorMessageContent = "Sorry, I encountered an error. Please check that your API key is correct and valid. You can reset it using the settings icon in the header.";

      if (error?.message && (error.message.includes('429') || error.message.toUpperCase().includes('RESOURCE_EXHAUSTED'))) {
        errorMessageContent = "It looks like the API key has exceeded its free usage limit (rate limit). This can happen if the key is being used by too many people.\n\nFor the best experience, each user should get their own free API key from Google AI Studio.\n\nYou can reset your current key using the settings icon in the header.";
      }
